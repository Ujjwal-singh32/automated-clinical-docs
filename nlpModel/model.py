...
# Load SciSpacy NER model (pretrained for biomedical NER)
nlp = spacy.load("en_ner_bc5cdr_md")  # install this model first

# Set Gemini API Key
genai.configure(api_key="AIzaSyBAUcrwMa7vatzC-fwIqVgMpGb9_7vr2rg")

# Hindi Doctor-Patient Conversation
conversation_hindi = """
Namaste doctor, mujhe 4 din se bukhaar aa raha hai aur gala bhi dard kar raha hai.
Kya aapko khaansi, sardardi ya thakan bhi ho rahi hai?
Haan, thodi khaansi hai aur pura sharir halka sa dard karta hai.
Aapka gala kaafi laal aur suja hua lag raha hai. Aapko Tonsillitis hua hai.
Kya yeh serious hai?
Nahi, agar time se dawai lein to 5–6 din mein thik ho jaayega.
Main aapko kuch dawaiyaan likh raha hoon:

Azithromycin 500 mg – roz ek baar subah, 3 din ke liye

Paracetamol 650 mg – jab bukhaar ho, 6-8 ghante ke antar par

Betadine gargle – din mein 2 baar gargle ke liye

Thik hai doctor, koi test bhi karwana hoga kya?
Haan, precaution ke liye ek CBC aur Throat Swab Culture test kara lijiye.
Aur khaane mein kuch parhez?
Thanda pani, ice cream, aur spicy khana avoid kijiye.
Aur rest zaruri hai – kam se kam 2 din ka aaram lijiye.
Prescription main likh raha hoon, test report le aayiye to main dobara check kar lunga.
"""

# Step 1: Translate to English
translator = Translator(from_lang="hi", to_lang="en")
lines = conversation_hindi.strip().split("\n")
conversation_english = "\n".join([translator.translate(line) for line in lines])

print("\n--- Translated English Conversation ---\n")
print(conversation_english)

# Step 2: Combine text
combined_text = conversation_english.replace("\n", " ")
...
grouped_entities = {}
for ent in doc.ents:
    if ent.label_ not in grouped_entities:
        grouped_entities[ent.label_] = set()
    grouped_entities[ent.label_].add(ent.text)
